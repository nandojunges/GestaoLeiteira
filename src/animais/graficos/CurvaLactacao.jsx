...
export default function CurvaLactacao({ idAnimal, dataParto }) {
  ...
  useEffect(() => {
    if (!idAnimal) return;
    buscarPorId('leite', idAnimal).then((lista) => {
        const ordenado = (Array.isArray(lista) ? lista : []).sort(
          (a, b) => new Date(a.data) - new Date(b.data)
        );
        setDados(ordenado);
        const lactacoes = Array.from(
          new Set(ordenado.map((d) => d.lactacao))
        ).sort((a, b) => a - b);
  ...
  }, [lactacaoSelecionada, dados, dataParto]);
