...
export default function AcaoParto({ vaca, onFechar }) {
  ...
  const salvarMaeTemp = () => {
    ...
    // Atualiza o statusReprodutivo para "pos-parto" no localStorage:
  ...
  const salvarMaeEBezerro = (dadosBezerro) => {
    const animais = JSON.parse(localStorage.getItem("animais") || "[]");
    const index = animais.findIndex(a => a.numero === vaca.numero);
    if (index !== -1) {
      if (!animais[index].partos) animais[index].partos = [];
      animais[index].partos.push(dadosMaeTemp);
      animais[index].ultimoParto = dadosMaeTemp.data;
      animais[index].numeroPartos = (parseInt(animais[index].numeroPartos || "0") + 1).toString();

      const bezerros = JSON.parse(localStorage.getItem("bezerros") || "[]");
      bezerros.push({ ...dadosBezerro, mae: vaca.numero });
      localStorage.setItem("bezerros", JSON.stringify(bezerros));
