...
export default function CadastroBasicoAnimal({ animais, onAtualizar }) {
  ...
  useEffect(() => {
    const salvas = localStorage.getItem("racasAdicionais");
    if (salvas) setRacasAdicionais(JSON.parse(salvas));
  }, []);
  ...
  const adicionarNovaRaca = () => {
    if (novaRaca.trim() && !racasAdicionais.includes(novaRaca)) {
      ...
      localStorage.setItem("racasAdicionais", JSON.stringify(atualizadas));
      setRaca(novaRaca);
      setNovaRaca("");
  ...
  const salvarAnimal = (complementares = {}) => {
    ...
    const atualizados = [...animais, novo];
