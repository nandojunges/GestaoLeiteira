...
import { carregarAnimaisDoLocalStorage } from './utilsAnimais';
...
function calcularDEL(ultimoParto) {
  if (!ultimoParto || ultimoParto.length !== 10) return '—';
  const [dia, mes, ano] = ultimoParto.split('/');
  const dataParto = new Date(`${ano}-${mes}-${dia}`);
  const hoje = new Date();
  const diff = Math.floor((hoje - dataParto) / (1000 * 60 * 60 * 24));
  return diff >= 0 ? diff : '—';
}
...
export default function ConteudoPlantel() {
  ...
  const vacasFiltradas = vacas
    ...
    .filter(v => {
      const atendeCategoria = !filtroCategoria || v.categoria === filtroCategoria;
      ...
      {animalSelecionado && (
        <ModalEditarAnimal
          ...
          onSalvar={() => {
            ...
            setVacas(carregarAnimaisDoLocalStorage());
          }}
        />
      )}
