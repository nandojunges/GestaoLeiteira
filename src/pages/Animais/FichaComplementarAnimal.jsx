...
export default function FichaComplementarAnimal({ onFechar, onSalvar }) {
  ...
  useEffect(() => {
    const chaves = Object.keys(localStorage).filter(k => k.startsWith("touro_"));
    const nomes = chaves.map(k => k.replace("touro_", ""));
    setTourosDisponiveis(nomes);
  }, [modalImportar]);
  ...
  const salvarCompleta = () => {
    ...
    const ficha = localStorage.getItem("touro_" + nomeTouro);
    ...
    const dados = {
      ...
      ultimoParto,
      dataPrevistaParto,
      fichaTouro: ficha ? JSON.parse(ficha) : null,
      numeroPartos: parseInt(quantidadePartos || 0),
      iaAnteriores: historico.filter(h => h.tipo === "IA").map(h => ({
        data: h.data,
        tipo: "IA",
        touro: nomeTouro || "—",
        inseminador: "—",
      ...
      partosAnteriores: historico.filter(h => h.tipo === "Parto").map(h => ({
        data: h.data,
        tipo: "Parto",
        obs: "—"
      })),
      secagensAnteriores: historico.filter(h => h.tipo === "Secagem").map(h => ({
        data: h.data,
        tipo: "Secagem",
        obs: "—"
      }))
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginBottom: '1.5rem' }}>
        ...
        <div>
          <label>Quantidade de Partos</label>
          <input ref={el => refs.current[4] = el} type="number" min="0" value={quantidadePartos} onChange={(e) => setQuantidadePartos(e.target.value)} onKeyDown={(e) => handleKey(e, 4)} style={inputStyle} />
        </div>
      </div>
