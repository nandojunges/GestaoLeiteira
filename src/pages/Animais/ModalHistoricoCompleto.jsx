...
export default function ModalHistoricoCompleto({ animal, onClose }) {
  ...
  useEffect(() => {
    ...
    const ias = JSON.parse(localStorage.getItem('inseminacoes') || '[]');
    const dxs = JSON.parse(localStorage.getItem('diagnosticos') || '[]');
    const secs = JSON.parse(localStorage.getItem('secagens') || '[]');
